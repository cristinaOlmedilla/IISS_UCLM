...
public class DepthLimited extends SearchAlgorithm {
	...
	public void doSearch() {
		...
		// LIFO QUEUE
		Deque<Node> openNodes = new ArrayDeque<Node>();
		...
		while ((openNodes.peek() != null) && !solutionFound) {
			...
			if (!exploredStates.contains(actualNodeState)) {
				...
				} else if (actualNode.getDepth() < limit) {
	...
	/**
	 * Method that it return the path follow from the root, or the beginning of the
	 * path follow by a Node. It saves in a List the sequence of parents giving a
	 * Node
	 */
